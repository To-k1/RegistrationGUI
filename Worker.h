...
public:
	...
	Worker(const Worker& src) : is_paused(src.is_paused) {}
	~Worker() {}
...
private:
    static bool handle_pause(Worker* worker = nullptr);
	static void send_process(const string processingName, const int processingNum, const int allNum, Worker* worker = nullptr);
	bool is_paused;
};
